...
- **规模**：13 试卷 ｜ 124 题目 ｜ **11 章节（按学校参考书目录）** ｜ 92 知识点 ｜ 604 证据 ｜ 51 相似题关系 ｜ 37 公式 ｜ 56 资料源

> **章节口径（2026-09-03 重划，按参考书目录）**：`chapters` 表 11 章为 `1总论 / 2统计数据的采集 / 3统计数据的整理 / 4统计数据的描述 / 5抽样与参数估计 / 6参数假设检验 / 7方差分析 / 8相关与回归分析 / 9时间序列分析 / 10统计指数 / 11国民经济统计基础知识`。旧12章结构已废弃：原"统计学导论/概率论基础"并入 ch1，原"综合指标"归 ch4，原"假设检验/非参数/方差分析"拆为 ch6/ch7，原"统计综合应用"改名 ch11。查询章节请一律用新 chapter_id。
...
sources (56个原始文件)
   └─< question_sources >─┐                          papers (13套试卷)
                          │                            └─< questions (124道题)
knowledge_evidence (604条证据) ──> knowledge_points (92知识点) ──> chapters (11章,按参考书目录)
                          │              │
