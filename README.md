...
- **章节口径**：数据库按**学校参考书目录**划为 **11 章**（见 §5 第 6 条），与期末题/习题集里"资料自带教材章节号"无关。
- **设计原则**：
...
│   └── reports/                 # 四份主题分析报告（Markdown，按下文 §5.1 阅读）
...
6. **章节口径（11 章，参考书目录）**：`chapters` 表共 11 章 —— `1总论 / 2统计数据的采集 / 3统计数据的整理 / 4统计数据的描述 / 5抽样与参数估计 / 6参数假设检验 / 7方差分析 / 8相关与回归分析 / 9时间序列分析 / 10统计指数 / 11国民经济统计基础知识`。此前的 12 章自定义结构已废弃；旧"综合指标"→新 ch4、旧"假设检验/非参数/方差分析"→新 ch6/ch7、旧"统计综合应用"→改新 ch11。**任何按章节的查询必须使用新 chapter_id**。

### 5.1 主题分析报告（`docs/reports/`）

仓库附带四份按当前 11 章口径生成的 Markdown 报告，供快速掌握命题规律，阅读顺序：**03 知识点考频（背什么）→ 02 章节考频（时间分配）→ 01 数据质量（可信度）→ 00 资料清单（文件索引）**。

- `00_资料清单.md` — 56 个资料文件清点 + 归档结构
- `01_数据质量检查.md` — 全量快照、年份完整度、回忆版质量分级、已知缺口
- `02_章节考频分析.md` — 11 章考频汇总、题型交叉表、覆盖年趋势
- `03_知识点考频分析.md` — TOP30 知识点、S/A 优先级、92 知识点按 11 章全排、跨年相似模板链

> 报告数据均从数据库 `select` 实时生成，脚本在原始工作区的 `08_脚本/`（`report_stats_new.py`、`report_importance_new.py`），非本仓库必需。
